...
public class SparseResultSetFactory implements ResultSetFactory {
  ...
  /**
   * The current standard client-server for a paged OAE search returns a "total" value
   * in the response. This may not be an exact count, but should be enough to let
   * client-side code set up a reasonable paging UX.  With a Solr search, "total" is
