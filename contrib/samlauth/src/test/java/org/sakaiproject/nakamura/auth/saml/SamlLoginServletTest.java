...
import org.sakaiproject.nakamura.auth.saml.SamlAuthenticationHandler;
...
public class SamlLoginServletTest {
  ...
  public void setUp() throws Exception {
    ...
    servlet = new SamlLoginServlet(handler, trustedTokenService);
  ...
  public void redirectWithoutTarget() throws Exception {
    when(request.getAuthType()).thenReturn(SamlAuthenticationHandler.AUTH_TYPE);

    ArgumentCaptor<String> redirectCaptor = ArgumentCaptor.forClass(String.class);

    when(request.getContextPath()).thenReturn("someContextPath");
    when(request.getAttribute(SamlAuthenticationHandler.AUTHN_INFO)).thenReturn(null);
    servlet.service(request, response);
  ...
  public void authAndAddToken() throws Exception {
    ...
    when(request.getAttribute(SamlAuthenticationHandler.AUTHN_INFO)).thenReturn(authnInfo);
    when(request.getRequestURI()).thenReturn("someURI");
    when(request.getAttribute(Authenticator.LOGIN_RESOURCE)).thenReturn("greatplace");
    servlet.service(request, response);
