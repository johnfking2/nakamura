...
class TC_Kern2122 < Test::Unit::TestCase
  ...
  def test_add_and_remove_viewers
    ...
    managed_group = @um.create_group "group-#{m}"
    assert_not_nil(managed_group, "expected managed group to be created")
    #add_manager_res = managed_group.add_manager @s, viewer1.name
    #assert_equal("200",add_manager_res.code, "adding manager should have succeeded")
    ...
    viewers = json["viewers"]
    assert_equal(3, viewers.length, "should be 2 viewers because viewer1 cannot remove viewer2" )
